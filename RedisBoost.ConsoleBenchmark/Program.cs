...
namespace RedisBoost.ConsoleBenchmark
{
	class Program
	{
		...
		private static IRedis[] _clients;

		static void Main(string[] args)
		{
			var testCase = ChucNangTuongTac();
		...
		private static TestCase ChucNangTuongTac()
		{
			int temp;
			Console.WriteLine("Nhap so lan kiem tra duoc thuc hien /default is " + Iterations);
			if (int.TryParse(Console.ReadLine(), out temp)) Iterations = temp;

			Console.WriteLine("Nhap so lan kiem tra trong goi /default is " + LoopSize);
			if (int.TryParse(Console.ReadLine(), out temp)) LoopSize = temp;

			while (true)
			{
				Console.Write(@"Chon 1 trong cac truong hop thu nghiem:
0. Small packet
1. Medium packets
2. Large packets
3. Mixed packets
...
");
				...
				Console.WriteLine("Khong biet truong hop. Thu 1 so khac ");
			}
		}
		...
		private static int RunSyncExecutionTest(string payload, IRedis testClient, int loopSize)
		{
			using (testClient)
			{
		...
		private static int RunBasicTest(IRedis testClient, string payload, int loopSize)
		{
			using (testClient)
			{
				testClient.Connect(_cs);
		...
		private static int RunMixedPackTest(IRedis testClient, int loopSize, string key1, string key2, bool flushdb = true)
		{
			using (testClient)
			{
		...
		private static int RunManyClientsTest(IRedis client, int iterations)
		{
			var rand = new Random();
			_clientsRunning = 0;
			//warm up
